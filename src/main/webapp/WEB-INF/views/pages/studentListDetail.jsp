                        ...
                        <form class="layui-form layui-form-pane" id="myFrom">
                            <table id="dynamic-table"
                                ...
                                <tr class="border">
                                    ...
                                    <td align="center">学生ID：${temp.id}<input type="hidden"
                                                                             value="${temp.id}"
                                                                             name="id" id="fuwuId">
                                    </td>
                                    <td align="center">学生名：${temp.stundentName}</td>
                                ...
                                <tr class="border">
                                    ...
                                    <td align="center">所属琴行：${teacher.belongtoPianoShop}</td>
                                    <td align="center">所属家长:${temp.parentName}</td>
                                    <td align="center">家长ID: ${temp.parentId}</td>
                                </tr>
                                <tr class="border">
                                    <td align="center">所属老师：${temp.teacherName}</td>
                                    <td align="center">老师ID:${temp.teacherId}</td>
                                    <td align="center">积分: ${elecPlayer.score}</td>
                                    <td align="center">游戏积分: ${temp.gameScore}</td>
                                </tr>
                                <tr class="border">
...
<script>
    layui.use(['upload', 'form', 'layer'], function () {
        ...
        form.on('submit(reg)', function (data) {
            ...
            $('form[id=myFrom]').attr('action', '${pageContext.request.contextPath}/huiYuanDetail/StudentsDetailUpdate.page');
        });
        ...
        $("#datade1").click(function (data) {
            ...
            layer.confirm('当前会员已经产生真实数据，确定删除？', function (index) {
                ...
                $.ajax({
                    url: '${pageContext.request.contextPath}/huiYuanDetail/StudentsDelete.page?id=' + fuwuId,
                    success: function (result) {
                        console.log(result)
                        if (result.code == 0) {
                            // obj.del();
                            layer.msg("删除成功！");
                            window.location.href="${pageContext.request.contextPath}/get/getStudent.page";
                        } else {
