...
//	}
	 ...
	 * @return UnifiedorderResult
	 */
	public static UnifiedorderResult payUnifiedorder(Unifiedorder unifiedorder,String key, String url){
		Map<String,String> map = MapUtil.objectToMap(unifiedorder);
		if(key != null){
			String sign = SignatureUtil.generateSign(map,key);
			unifiedorder.setSign(sign);
...
//	 * 注意�?
//	 *	1.交易时间超过半年的订单无法提交�??款；
//	 *	2.微信支付�?款支持单笔交易分多次�?款，多次�?款需要提交原支付订单的商户订单号和设置不同的�?款单号�?�一笔�??款失败后重新提交，要采用原来的�??款单号�?��?��??款金额不能超过用户实际支付金额�??
...
//	/**
//	 * 撤销订单
//	 * 7天以内的交易单可调用撤销，其他正常支付的单如�?实现相同功能请调用申请�??款API。提交支付交易后调用【查询订单API】，没有明确的支付结果再调用【撤�?订单API】�??<br>
//	 * 官方�?术文档已撤销
...
//	 * 查询�?�?
//	 *
//	 * 提交�?款申请后，�?�过调用该接口查询�??款状态�?��??款有�?定延时，用零钱支付的�?�?
//	 * 20 分钟内到账，银行卡支付的�?�?3 个工作日后重新查询�??款状态�??
...
//	 * 现金红包 <br>
//	 *
//	 * 1.发�?�频率限�?------默认1800/min <br>
//	 * 2.发�?�个数上�?------按照默认1800/min�?<br>
//	 * 3.金额上限------根据传入场景id不同默认上限不同，可以在商户平台产品设置进行设置和申请，�?大不大于4999�?/�?<br>
//	 * 4.其他的�?�量”上的限制还有哪些？------用户当天的领取上限次�?,默认�?10<br>
//	 * 5.如果量上满足不了我们的需求，如何提高各个上限�?------金额上限和用户当天领取次数上限可以在商户平台进行设置<br>
//	 * �? <br>
//	 * 1：如果你是服务商，希望代你的特约商户发红包，你可以申请获得你特约商户的�?�现金红包产品授权�?��?�操作路径如下：【登录商户平�?-产品中心-
//	 * 特约商户授权产品】（即将上线�? <br>
//	 * 2：红包金额大�?200时，请求参数scene_id必传
//	 * 
//	 * @param sendredpack
//	 *            sendredpack
//	 * @param key
...
//	/**
//	 * 裂变红包 <br>
//	 * �?次可以发放一组红包�?�首先领取的用户为种子用户，种子用户领取�?组红包当中的�?个，并可以�?�过社交分享将剩下的红包给其他用户�?�裂变红包充分利用了人际传播的优势�??
...
//	/**
//	 * 查询红包记录 <br>
//	 * 用于商户对已发放的红包进行查询红包的具体信息，可支持普�?�红包和裂变包�??
//	 * @since 2.8.5
...
//	 * 企业付款 <br>
//	 * 接口调用规则�?<br>
//	 * 给同�?个实名用户付款，单笔单日限额2W/2W<br>
//	 * 给同�?个非实名用户付款，单笔单日限�?2000/2000<br>
//	 * �?个商户同�?日付款�?�额限额100W<br>
//	 * 单笔�?小金额默认为1�?<br>
//	 * 每个用户每天�?多可付款10次，可以在商户平�?--API安全进行设置<br>
//	 * 给同�?个用户付款时间间隔不得低�?15�?<br>
//	 * 
//	 * @param transfers
//	 *            transfers
//	 * @param key
