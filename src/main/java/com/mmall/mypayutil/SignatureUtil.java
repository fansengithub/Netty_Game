...
public class SignatureUtil {
	 ...
	 *            map
	 * @param sign_type
	 *            HMAC-SHA256和MD5 加密 toUpperCase
	 * @param paternerKey
	 *            paternerKey
	 * @return sign
	 */
	public static String generateSign(Map<String, String> map,
			String sign_type, String paternerKey) {
		Map<String, String> tmap = MapUtil.order(map);
		...
		if ("HMAC-SHA256".equalsIgnoreCase(sign_type)) {
			try {
				...
				SecretKeySpec secret_key = new SecretKeySpec(
						paternerKey.getBytes("UTF-8"), "HmacSHA256");
				sha256_HMAC.init(secret_key);
				return Hex.encodeHexString(
						sha256_HMAC.doFinal((str + "&key=" + paternerKey)
			...
			return DigestUtils.md5Hex(str + "&key=" + paternerKey)
					.toUpperCase();
		}
	}
...
//	}
	 ...
	 *            payFeedback
	 * @param paySignKey
	 *            公众号支付请求中用于加密的密钥Key, 可验证商户唯�?身份,对应于支付场景中�? appKey �?
	 * @return boolean
	 */
...
//				"appsignature", "signmethod");
//		return payNotify.getAppsignature().equals(
//				generatePaySign(map, paySignKey));
//	}
...
//	}
	...
	/**
	 * 验证 mch pay notify sign 签名 使用更好�? SignatureUtil.validateSign
	 * 
	 * @param mchPayNotify
	 *            mchPayNotify
	 * @param key
	 ...
	 */
//	@Deprecated
//	public static boolean validateAppSignature(MchPayNotify mchPayNotify,
//			String key) {
...
//	}
	...
	/**
	 * 验证代扣签约 签名 使用更好�? SignatureUtil.validateSign
	 * 
	 ...
	 */
//	@Deprecated
//	public static boolean validateAppSignature(
//			PapayEntrustwebNotify papayEntrustwebNotify, String key) {
...
//	}
	...
	/**
	 * mch 支付、代扣异步�?�知签名验证�? 该方法可以替�? mch 支付、代扣异步�?�知验证，用以防止官方返回参数与bean不一至�?�导致签名错误�??
	 * 
	 * @param map
	 *            参与签名的参�?
	 * @param key
